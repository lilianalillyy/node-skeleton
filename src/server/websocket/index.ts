...
import { WebsocketMiddleware, WebsocketMiddlewarePromise, WebSocketRouteOptions } from "../../types/websocket";
...
>(
    ...
    if (!util.types.isAsyncFunction(middleware)) {
        return new Promise<boolean>((resolve) => middleware(conn, req, fasteer, resolve));
    }

    return await (middleware as WebsocketMiddlewarePromise)(conn, req, fasteer);
};

export const middlewares = async <TFastifyInstance extends FastifyInstance = FastifyInstance>(
    conn: SocketStream,
    req: FastifyRequest,
    ...
    (
        ...
        server.route({
            ...
            async wsHandler(conn, req) {
                ...
                const routeMiddlewares = middleware ? (Array.isArray(middleware) ? middleware : [middleware]) : [];
 ...
 */
export const socketSend = (socket: { send: SocketStream["socket"]["send"] }, payload: object | string): void =>
 ...
 */
export const decorateWebsocket = <TFastify extends FastifyInstance = FastifyInstance>(
    fastify: TFastify,
    globalMiddlewares: WebsocketMiddleware[] = []
) => {
